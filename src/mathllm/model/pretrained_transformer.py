...
class GPT2(nn.Module):
    ...
    def __init__(self, model, save_dir=None):
        ...
        config = CausalConfig(**config_args)
        ...
        if save_dir is not None:
            assert isinstance(save_dir, PosixPath), "save_dir must be a PosixPath object"
        ...
        for k in special:
            if 'query' in k:
                if 'bias' in k:
                    ...
                    local_state_dict[k] = gpt_state_dict[special[k]][:, :self.emb_dim].T
            elif 'key' in k:
                if 'bias' in k:
                    local_state_dict[k] = gpt_state_dict[special[k]][self.emb_dim:2*self.emb_dim]
                else:
                    local_state_dict[k] = gpt_state_dict[special[k]][:, self.emb_dim:2*self.emb_dim].T
            elif 'value' in k:
                if 'bias' in k:
                    local_state_dict[k] = gpt_state_dict[special[k]][2*self.emb_dim:]
                else:
                    local_state_dict[k] = gpt_state_dict[special[k]][:, 2*self.emb_dim:].T
...
class Mistral(nn.Module):
    ...
    def __init__(self, save_dir=None):
        ...
        logger.info(f"Loading Mistral model from mistral checkpoint")
    ...
    def load_from_mistral(self):
        ...
        try:
            ...
            raise ValueError(f"Could not load Mistral weights from {self.save_dir}.\nYou can download them online.")
        ...
        logger.info(f"Loading Mistral tokenizer from mistral checkpoint")
        self.tokenizer = SentencePieceProcessor(model_file=str(self.save_dir / 'tokenizer.model'))
